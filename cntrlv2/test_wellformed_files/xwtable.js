				...
				for (var i = 0; i < tbl_column_count; i++) {
					...
					if (i ==1) {
						...
						if (tbl_columns[i].id.search(/custom/) == -1) {
							...
							var col_children =  tbl_columns[i].childNodes;
								for (var y = 0; y < col_children.length; y++) {
									if (col_children[y].tagName=='A') {
									  if (col_children[y].childNodes[0].nodeType==3) {
										 table_col_name = col_children[y].childNodes[0].nodeValue;
										}
									}				
								}
				...
				*/		
				...
				keycodeHandler: function(e) {
					 ...
					 if (code == 13) {
					 	...
					 	/* the query column can be determine by getting the TH columns sort value */
					 	filterCells = document.getElementById("tblFilter").getElementsByTagName("td");
					 	//iterate over the filter cell columns
					 	for (var i = 0; i < filterCells.length; i++) {
						   //with each cell iterate over childNodes to make sure with a link
						   for (var n = 0; n < filterCells[i].childNodes.length; n++) {									 	 
						 	 if (filterCells[i].childNodes[n].id && filterCells[i].childNodes[n].id == k.id) {
						 	     TH_href = document.getElementById("xw_col_titles").getElementsByTagName("a")[i].href;
						 	    break;	
						 	 }
					 	    }
						}
					 	
					 	//now get the sort column from the href
					 	var dbColName = xw.UTIL.URLgetValueFromKey(TH_href, "tblsort");	
					 	// get the value of the field 
					 	var fieldValue = document.getElementById(k.id).value;
					 	
					 	//alert("dbColName: " + dbColName + " fieldValue: " + fieldValue);
					 	// set the filter
