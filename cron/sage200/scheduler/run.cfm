...
--->
    <cfquery name="task" datasource="veappdata">
       SELECT t.id,
	   t.crontaskid,
	   t.server,
	   c.name,
...
--->
    <cfif !task.recordCount>
        <cfoutput>Nothing to do</cfoutput>
<!--- Nothing more to do. --->
        <cfexit />
...
--->
    <cfquery name="changeTaskStatus" datasource="veappdata">
        UPDATE
...
--->
    ...
    <cfquery name="iCronAction" datasource="veappdata">
        INSERT INTO
...
<!--- fetch the id--->
    <cfquery name="action" datasource="veappdata">
       SELECT
       id
       FROM
...
--->
        <cfset taskData = {
            ...
            template = replace(task.template,"\\","\"),
...
--->
    ...
    <cfif dateOfNextExecution lte now()>
        <cfset dateOfNextExecution = now() + task.interval />
    </cfif>
...
<!---
    ...
    next execution.
--->
        <cfquery name="updateTask" datasource="veappdata">
            UPDATE
                veappdata.tblCronSchedule
            SET
                <!---
                ...
                <!---
                    ...
                    difference.
                --->
             dateOfNextExecution = <cfqueryparam value="#dateOfNextExecution#" cfsqltype="cf_sql_timestamp" />,
...
--->
        <cfquery name="updateAction" datasource="veappdata">
               UPDATE
                veappdata.tblCronAction
                SET
...
--->
            <cfquery name="logError" datasource="veappdata">
                UPDATE
                     veappdata.tblCronAction
                SET
