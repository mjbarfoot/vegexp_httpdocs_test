...
VARIABLES.logFileName="custfavlog";
VARIABLES.accountid="LEHBRO2";
//time the task
...
// dev of prod mode?
VARIABLES.isProduction = false;//isProductionServer();
...
if (NOT isComplete) abortTask(VARIABLES.custfavlog);
...
if (isQuery(q)) {
	...
	abortTask(VARIABLES.custfavlog);
}
...
//isComplete = ftpToWebServer("custfavOut.xml", VARIABLES.custfavlog, "favourites.xml");
if (NOT isComplete) abortTask(VARIABLES.custfavlog);

// *** ftp the favourites.xml file to the VE Web Server
//isComplete = ftpToWebServer("dateLog.log", VARIABLES.custfavlog);
if (NOT isComplete) abortTask(VARIABLES.custfavlog);
...
// *** Job Done
VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# CRONJOB: getFavourites - COMPLETE. Duration #VARIABLES.task_tickinterval#  s");
...
<cftry>
	...
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# clearUpFavDB Successful (#tickinterval# s)");
...
<cfcatch type="any">
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & formattedError);
	</cfscript>	
...
<cftry>
	...
	AND SCA.CUSTOMERACCOUNTNUMBER = '#VARIABLES.ACCOUNTID#'
	AND SOR.DOCUMENTSTATUSID = 2 
	AND SOR.REQUESTEDDELIVERYDATE >= <CFQUERYPARAM CFSQLTYPE="CF_SQL_TIMESTAMP" VALUE="#DATEADD('D',ARGUMENTS.DAYS,NOW())#">
	</cfquery>
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Success: qGetOrders - fetched #qGetOrders.recordcount# records in #tickinterval# ms");
	ret = qGetOrders;
	</cfscript>
...
<cfloop query="ARGUMENTS.qGetOrders">
	...
	<cfif currentrow eq ARGUMENTS.qGetOrders.recordcount>
		...
		tickEnd=getTickCount();
		tickinterval=decimalformat((tickend-tickbegin)/1000);
		VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Success: putSalesOrdersInFavDB - inserted #i# records in #tickinterval# ms");	
		ret = true;
		</cfscript>
	</cfif>
</cfloop>
...
<cfcatch type="any">
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Error: getSalesOrderLinesFromSage failed trying to get Orders from tblSOPOrder, erroring row was #error_row#, tried to insert values #error_values#, error details: #error_text#");
...
<cfloop query="g">
	...
	<cfcatch type="database">
		...
		tickEnd=getTickCount();
		tickinterval=decimalformat((tickend-tickbegin)/1000);
		VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Error: getSalesOrderLinesFromSage getting records from SOPOrderReturnLine, erroring row was #error_row#, tried to insert values #error_values#, error details: #error_text#");
		</cfscript>
...
<cfloop query="q">
	...
	<cfif currentrow eq q.recordcount>
		...
		tickEnd=getTickCount();
		tickinterval=decimalformat((tickend-tickbegin)/1000);
		VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Success: putSalesOrderItemsInFavDB - inserted #i# records (#tickinterval# s)");
		ret = true;
		</cfscript>
	</cfif>	
</cfloop>	
...
<cftry>
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: getOrderItems - selected #qGetOrderItems.recordcount# records (#tickinterval# s)");
	ret=qGetOrderItems;
	</cfscript> 
...
<cfif currentrow eq ARGUMENTS.q.recordcount>
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: copyToTblOrderItem  - inserted #i# records (#tickinterval# s)");
	ret=true;
	</cfscript> 
</cfif>
...
<cftry>
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: getGroupedAccountAndStockCodes -  selected #q.recordcount# records (#tickinterval# s)");
	ret=q;
	</cfscript> 
...
<cftry>
	...
	<cfif currentrow eq ARGUMENTS.q.recordcount>
		...
		tickEnd=getTickCount();
		tickinterval=decimalformat((tickend-tickbegin)/1000);
		VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: copyGroupedDataToTblFavourites - inserted #ARGUMENTS.q.recordcount# records (#tickinterval# s)");
		ret= true;
		</cfscript> 
	</cfif>
...
<!--- get all sales order lines for last 6 months organised by customer account ref and stockcode with qty to date and order count --->
	...
	AND C.CUSTOMERACCOUNTNUMBER = '#VARIABLES.ACCOUNTID#'
	...
	<cfscript>
		tickEnd=getTickCount();
		tickinterval=decimalformat((tickend-tickbegin)/1000);
		VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: getQtyToDateForLastSixMonthsFromSage - selected #qGetQtyToDate.recordcount# records (#tickinterval# s)");
		ret	= qGetQtyToDate;
	</cfscript> 
...
<cftry>
	...
	<cfif currentrow eq gFavs.recordcount>
		...
		tickEnd=getTickCount();
		tickinterval=decimalformat((tickend-tickbegin)/1000);
		VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: updateQtyToDate - updated #i# records (#tickinterval# s)");
		ret=true;
		</cfscript> 
	</cfif>
...
<cftry>
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: getLastOrderQuantity - selected #q.recordcount# records (#tickinterval# s)");
	ret=q;
	</cfscript> 
...
<cftry>
	...
	<cfif currentrow eq ARGUMENTS.q.recordcount>
		...
		tickEnd=getTickCount();
		tickinterval=decimalformat((tickend-tickbegin)/1000);
		VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: updateLastOrderQuantity - updated #ARGUMENTS.q.recordcount# records (#tickinterval# s)");
		ret= true;
		</cfscript>
	</cfif>
...
<cftry>
	...
	tickEnd=getTickCount();
	tickinterval=decimalformat((tickend-tickbegin)/1000);
	VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# " & "Success: cleanNullFavourites (#tickinterval# s)");
	ret=true;
	</cfscript> 
...
tickEnd=getTickCount();
tickinterval=decimalformat((tickend-tickbegin)/1000);
VARIABLES.favOut.write(trim(xmlFavaourites));
VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Success: writeFavourites - Wrote Favourites XML to filesystem (#tickinterval# s)");
</cfscript>
...
tickEnd=getTickCount();
tickinterval=decimalformat((tickend-tickbegin)/1000);
VARIABLES.dateLog.write("#now()#");
VARIABLES.dateLog.write(VARIABLES.days);
VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Success: writeDateLog - Wrote dateLog.log to filesystem. (#tickinterval# s)");
ret=true;
</cfscript>
<cfcatch type="any">
tickEnd=getTickCount();
tickinterval=decimalformat((tickend-tickbegin)/1000);	
VARIABLES.custfavlog.write("Error: writeDateLog - Failed to write dateLog.log to filesystem (#tickinterval# s)");
ret=false;
</cfcatch>
</cftry>
<cfreturn ret/>
...
case "localhost": VARIABLES.AppMode="development";
...
<cftry>
<cffile action = "delete"  file = "#VARIABLES.logpath#custfavOut.xml">
<cffile action = "delete"  file = "#VARIABLES.logpath#custdateLog.log">
<cffile action = "delete"  file = "#VARIABLES.logpath#custfavlog.log">
<cfcatch type="any"></cfcatch>
</cftry>

<cfscript>
//create log files
VARIABLES.custfavlog 			= createObject("component", "logwriter").init(VARIABLES.logpath, "custfavlog", VARIABLES.logtype);
VARIABLES.custfavOut			= createObject("component", "logwriter").init(VARIABLES.logpath, "custfavOut.xml", VARIABLES.logtype, true);
VARIABLES.custdateLog 		= createObject("component", "logwriter").init(VARIABLES.logpath, "custdateLog.log", VARIABLES.logtype, true);
...
job_tickEnd=0;
job_tickinterval=0;
VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# CRONJOB: getCustFavourites - started");
VARIABLES.custfavlog.write("#timeformat(now(), 'H:MM:SS')# Description: Retrieves Sage 200 order data from and analyses/creates new favourites data. FTPs data to server.");
</cfscript>
</cffunction>
