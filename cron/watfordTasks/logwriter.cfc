...
<cfcomponent output="false">
	...
	<cfargument name="suppressHeader" type="boolean" required="false" default="false" />
	
	<cfscript>
	
	this.dblogname 		= arguments.logfile;
	
	// if filename doesn't contain a period then use {logfilename_yyyy-mm-dd.log}, else use the filename passed
	if (find(".", ARGUMENTS.logfile) eq 0) {
		this.logfile 		= arguments.logdir &  arguments.logfile & "_" & dateformat(now(), "yyyy-mm-dd") & ".log";
	} else {
		this.logfile 		= arguments.logdir &  arguments.logfile;
	}
	...
	if (NOT fileLogAction("LogExists")) fileLogAction("create", "", ARGUMENTS.suppressHeader);	
	...
	<cfset var initOut = "" />
		...
		<cfcase value="create">
			<cfif ARGUMENTS.suppressHeader>
				<cfset initOut = ""/>
			<cfelse>
				<cfset initOut = "Log started: #dateformat(now(), 'dd/mm/yyyy')# #timeformat(now(), 'h:mm:ss tt')#"/>
			</cfif>		
