...
<!--- MESSAGES --->
	<cffunction name="sendMessage" access="public" returntype="string" output="false" hint="Attempts to send a message through the MailGun API - returns message if successul and throws an error otherwise">
		...
		<cfscript>
			...
			var files    = {};
			var postVars = {
				  from    = lcase(arguments.from)
				, to      = lcase(arguments.to)
				, subject = lcase(arguments.subject)
				, text    = arguments.text
				, html    = arguments.html
			};
...
<!--- PRIVATE HELPERS --->
	...
	<cfoutput>
		url: #_getRestUrl( arguments.uri, arguments.domain )# <br/>
		method: "#arguments.httpMethod#" <br/>
		header = Authorization" value="Basic #ToBase64(  'api:#_getApiKey()#' )#
		postvars = #arguments.postVars.toString()#
	</cfoutput>
