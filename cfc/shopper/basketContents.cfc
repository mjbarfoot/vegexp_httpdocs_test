...
<cfargument name="qty" type="numeric" required="false" default="1" />
    ...
    <taconite-root xml:space="preserve">
        ...
        <taconite-replace  contextNodeID="baskettotal" parseInBrowser="true">
            <div id="baskettotal">
                TOTAL: <span id="grandTotal" <cfif session.shopper.basket.getGrandTotal() lt session.auth.mov>class="movAlert"</cfif>><cfif SESSION.Auth.isLoggedIn>&##163; #trim(DecimalFormat(session.shopper.basket.getGrandTotal()))#</cfif></span>
            </div>
        </taconite-replace>
    </taconite-root>
    </cfoutput>
...
<cffunction name="clientSaveBasket" access="remote"  output="true" returntype="void">
<cfscript>
    APPLICATION.applog.write("#timeformat(now(), 'H:MM:SS')# In clientSaveBasket");
   // session.shopper.basket.BasketToWDDX();
</cfscript>
    <cfcontent type="text/xml" />
    <cfoutput>
    <taconite-root xml:space="preserve">
...
<td style="width:17px">(#quantity#)</td><td style="width:129px">#xmlformat(left(session.shopper.basket.getProductDescription(productID), 20))#</td><td style="width:31px"><cfif SESSION.Auth.isLoggedIn or SESSION.Auth.viewPrices>#decimalformat(session.shopper.basket.getTotal(productID))#</cfif></td>
		</tr></cfloop>
...
<cfset VARIABLES.myOldBasket = session.shopper.oldbasket.list()>
	...
	<!--- if there are items in the basket create the table, otherwise return empty div --->
		...
		<cfloop query="VARIABLES.myOldBasket" startrow="1" endrow="#VARIABLES.myOldBasket.recordcount#">
            ...
            <cfset portionCost = product.getPortionSize()/>
		<tr>
			<td>#xmlformat(product.getDescription())#</td>
			<td>#xmlformat(product.getPackSize())#</td>
			<td style="width:40px"><cfif portionCost neq "">#decimalformat(portionCost)#</cfif></td>
			<td id="prc_#ProductID#">&##163;#product.getSalePrice()#</td>
...
<cfset VARIABLES.myBasket = session.shopper.basket.list()>
	...
	<cfif VARIABLES.myBasket.recordcount gt 0>
		...
		<cfloop query="VARIABLES.myBasket" startrow="1" endrow="#VARIABLES.myBasket.recordcount#">
            <cfset product = session.shopper.basket.getProduct(ProductID)/>
            <cfset portionCost =  product.getPortionCost() />
		<tr>
			<td>#xmlformat(product.getDescription())#</td>
			<td>#xmlformat(product.getPackSize())#</td>
			<td style="width:40px"><cfif SESSION.Auth.isLoggedIn><cfif portionCost neq "">#decimalformat(portionCost)#</cfif></cfif></td>
			<td id="prc_#ProductID#"><cfif SESSION.Auth.isLoggedIn>&##163;#product.getSalePrice()#</cfif></td>
...
<cfset VARIABLES.myBasket = session.shopper.basket.list()>
	...
	<cfoutput>
		...
		<tr>
			<td>#xmlformat(product.getStockCode())#</td>
			<td>#xmlformat(product.getDescription())#</td>
			<td id="qty_#ProductID#" style="text-align:center;">#quantity#</td>
			<td id="unt_#ProductID#" style="text-align:center;">#xmlformat(product.getPackSize())#</td>
