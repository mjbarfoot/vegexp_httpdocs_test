...
<cfcomponent output="false">
    ...
    <cffunction name="getFilePath" output="false"  access="public" hint="Retrieves the filename and path">
        <cfreturn this.logfile />
     </cffunction>

	
	<cffunction name="write" output="false"  access="public" hint="Writes some given text to the log file">
	<cfargument name="content" type="string" required="true" />
    <cfargument name="logtype" type="string" required="false" default="INFO" />

    <cfscript>
	if (application.AppMode neq "production") {
		// call either fileLogAction or dbActionLog
		evaluate(this.logtype & 'LogAction("write", arguments.content, arguments.logtype)');
	}
	</cfscript>
	...
	<cfif application.AppMode neq "production">
		...
		<cfcase value="write">
			<cffile     action 		= "append" 
						file 		= "#this.logfile#" 
					    addnewline	= "true"
						output 		= "#dateformat(now(), 'dd/mm/yyyy')# #timeformat(now(), 'h:mm:ss tt')# #arguments.content#"
						charset 	= "utf8" >
	...
	<cfset var ret="" />
		...
		<cfcase value="read">
			...
			select logname, logtype, eventdate, event
			from tblEventLog
		...
		<cfcase value="write">
			...
			(logname, logtype, event,  eventdate)
			values ('#this.dblogname#','#arguments.logtype#','#arguments.content#','#dateformat(now(),'yyyy-mm-dd')# #timeformat(now(), 'hh:mm:ss.l')#')
			</cfquery>	
		</cfcase>
		</cfswitch>
	</cffunction>
